...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/external baselines/cloud_synthetic_split}
\caption{Synthetic cloud sliced by baseline family: (a) Pareto-core methods (Joint, SP-Lit, Multi-FR, TP-Lit), (b) operational extremes (VCC, OD). Axes auto-scaled per panel so VCC/OD's wide provider-cost range does not compress the Pareto-core into a sliver.}
\label{fig:cloud-split}
\end{figure}

Figure~\ref{fig:cloud} plots each baseline's provider cost rate (log scale) against average user time per rider across the shared regime grid. Each baseline sweeps its own structural hyperparameters internally: partition methods sweep $K \in \{2,\dots,6\}$, Multi-FR sweeps a (fleet, $\delta$) combo grid, OD and VCC sweep fleet. No fixed $K$ is imposed across the comparison. Some markers visually overlap when a baseline's chosen design does not respond to $w_r$ at a given $\Lambda$ (e.g.\ TP-Lit shows three points, one per $\Lambda$). Figure~\ref{fig:cloud-split} splits the same data into a Pareto-core panel and an operational-extremes panel; the split makes the Pareto-core's structure (which the merged plot crowds into the left band) directly readable.
...
TP-Lit occupies a narrow horizontal band at user time $\approx 0.20$ to $0.23$\,h with provider cost 233 to 244. Its cloud is nearly flat in both axes because the design is essentially $w_r$-insensitive: fixed $T{=}0.25$\,h and the consistently-picked fleet-20 design give realised wait $T/2 = 0.125$\,h regardless of $\Lambda$ or $w_r$. The three visible markers correspond to the three $\Lambda$ values (in-vehicle time varies mildly with candidate-stop proximity). Maintaining a fleet of 20 vehicles for 25 candidate stops makes the fleet-cost term alone ($c_f \times 20 \approx 80$) account for $\sim 35\%$ of provider cost.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/external baselines/cloud_pittsburgh_split}
\caption{Pittsburgh cloud sliced by baseline family.}
\label{fig:cloud-real-split}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/external baselines/cloud_four_cities_split}
\caption{Cross-city clouds sliced by baseline family. Rows are cities; left columns show Pareto-core methods, right columns operational extremes.}
\label{fig:four-city-cloud-split}
\end{figure}
